...
- Pokémon sprite images: `GET /icon/<name>` (already implemented in `app.py`), proxies to `https://img.pokemondb.net/sprites/silver/normal/<name>.png`.
- Currently `app.py` only has `/` (returns the full unpaginated DB dump) and `/icon/<name>` — pagination/sorting/filtering/capture endpoints still need to be built.
...
- `src/App.tsx` is still the default Vite scaffold (counter button, Vite/React boilerplate links) — none of the Pokédex UI has been built yet.
...
- **Saving/capturing**: let users mark a Pokémon as "captured"; persist the selection in server memory for the server's lifetime (in-memory server state is fine, no DB/file persistence required).
