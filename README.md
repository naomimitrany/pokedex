...
5. **Added a Pokémon detail page at `/pokemon/:name`.** Not explicitly required, but it's the natural target for clicking a grid card. Clicking a card carries that row's data along via router state so the detail page renders instantly on navigation, with a network fetch to `GET /pokemon/<name>` as the fallback (and background revalidation) for direct visits, refreshes, or shared links.
